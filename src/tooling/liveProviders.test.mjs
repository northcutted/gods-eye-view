...
test('live entry resolves in Node and aircraft normalization stays independently portable', async () => {
  ...
  assert.equal(normalizer.normalizeAdsbLolAircraftState, portable.normalizeAdsbLolAircraftState);
});

test('OpenSky state and track routes share tokens, retain cache and use regional fallback', async t => {
  environment(t, {
    OPENSKY_CLIENT_ID: 'fixture-client', OPENSKY_CLIENT_SECRET: 'fixture-secret',
    OPENSKY_AUTH_MODE: 'oauth', OPENSKY_USERNAME: undefined, OPENSKY_PASSWORD: undefined,
  });
  let now = Date.now();
  ...
  t.mock.method(globalThis, 'fetch', async (url, options) => {
    ...
    if (url.includes('/token')) return Response.json({ access_token: 'fixture-token', expires_in: 1800 });
    ...
    if (url.includes('/lat/')) return Response.json({ now: now / 1000, ac: [{ hex: 'abc123', lat: 30, lon: -97, alt_baro: 10000 }] });
    throw Error(`Unexpected URL: ${url}`);
  });
  const states = install(providers.openSkyProxy());
  assert.equal((await states('/api/opensky', '?lat=30&lon=-97')).statusCode, 200);
  assert.equal((await states('/api/opensky', '?lat=30&lon=-97')).headers['x-opensky-cache'], 'HIT');
  assert.equal(calls.length, 2);
  const tracks = install(providers.trackBackfillProxies(), true);
  assert.equal((await tracks('/api/opensky-track', '?icao24=ABC123')).statusCode, 200);
  ...
  assert.equal((await tracks('/api/adsblol/trace', '?hex=invalid')).statusCode, 400);
  now += 130_000;
  ...
  const fresh = await import(`../../server/providers/aircraft/opensky.js?fallback=${now}`);
  environment(t, { OPENSKY_AUTH_MODE: 'anon' });
  const fallback = await install(fresh.openSkyProxy())('/api/opensky', '?lat=30&lon=-97');
...
test('AIS preview route ingests through the socket, returns tracks and disposes before restart', async t => {
  ...
  environment(t, {
    ...
    AISSTREAM_BOUNDING_BOXES: undefined, AISSTREAM_MESSAGE_TYPES: undefined,
  ...
  upstream.on('connection', socket => {
    ...
    socket.on('message', raw => {
      ...
      for (const [lat, epoch] of [[30, 1_800_000_000], [30.01, 1_800_000_060]]) {
        socket.send(JSON.stringify({
          MessageType: 'PositionReport',
          MetaData: { MMSI: 123456789, latitude: lat, longitude: -97, time_utc: new Date(epoch * 1000).toISOString() },
          Message: { PositionReport: { UserID: 123456789, Sog: 10, Cog: 90, TrueHeading: 511 } },
        }));
      }
    });
  });
  ...
  assert.equal((await request('/api/ais-live', '/track?mmsi=bad')).statusCode, 400);
