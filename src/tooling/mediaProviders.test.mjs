...
test('provider composition forwards the application root to the CCTV catalog', async (t) => {
  isolate(t);
  const plugins = localProviderPlugins({
    sourceRoot: fixture(t, 'deployed-camera'),
    includeKeySetup: false,
    includeRealtimeDebugLog: false,
  });
  const request = install(
    plugins.find((plugin) => plugin.name === cctvProxy().name),
  );
  const response = await request('/sources');
  assert.equal(response.status, 200);
  assert.deepEqual(
    JSON.parse(response.body).sources.map((source) => source.id),
    ['deployed-camera'],
  );
});
